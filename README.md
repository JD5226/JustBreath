...
From Scenario 1 and Scenario 3, we found that when the person is not breathing, the humidity either remains the same as initial value or constantly decreases. That means we can use this as a start condition of 10-second counting.
...
2.	The humidity fluctuates in the range between <strong> IV-3</strong> and <strong>IV+3 </strong> . 
    When the fluctuation is in the range, we consider this as no breathing, so the timer starts to count (or keeps counting).
